...
draw:
    #!/usr/bin/env bash
    set -euo pipefail
    echo "Make sure to run mullvad, there's something blocked :("
    keymap -c "{{ draw }}/config.yaml" parse -z "{{ config }}/base.keymap" --virtual-layers Combos >"{{ draw }}/base.yaml"
    yq -Yi '.combos.[].l = ["Combos"]' "{{ draw }}/base.yaml"
    keymap -c "{{ draw }}/config.yaml" draw "{{ draw }}/base.yaml" -k "obosob/arch_36" >"{{ draw }}/base.svg"
    resvg "{{ draw }}/base.svg" "{{ draw }}/base.png"
